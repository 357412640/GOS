...
export const setTagNavListInLocalstorage = list => {
  localStorage.GOSTagNaveList = JSON.stringify(list)
}
/**
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getTagNavListFromLocalstorage = () => {
  const list = localStorage.GOSTagNaveList
...
/**
 *  获取从今天到之前的时间范围
 *  @param int start: 开始日距离今天的天数
 *  @param int end: 结束日距离今天的天数
 *  }
 */
export const getWeek = (start, end) => {
  ...
  const endTime = new Date(date - getDays(start))
  // console.log(endTime)
  ...
  const startTime = new Date(date - getDays(end))
